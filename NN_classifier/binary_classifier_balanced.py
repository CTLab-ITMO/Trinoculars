...
class SimpleBalancedNetwork(nn.Module):
    def __init__(self, input_size, semantic_indices, hidden_layers=[256, 128, 64], num_classes=2, dropout_rate=0.3):
        super(SimpleBalancedNetwork, self).__init__()
        
        self.semantic_indices = semantic_indices
        self.form_indices = [i for i in range(input_size) if i not in semantic_indices]
        
        semantic_size = len(semantic_indices)
        form_size = input_size - semantic_size
        
        self.semantic_network = nn.Sequential(
            nn.Linear(semantic_size, hidden_layers[0] // 2),
            nn.ReLU(),
            nn.Dropout(dropout_rate)
        )
        
        self.form_network = nn.Sequential(
            nn.Linear(form_size, hidden_layers[0] // 2),
            nn.ReLU(),
            nn.Dropout(dropout_rate)
        )
    ...
    def forward(self, x):
        semantic_features = x[:, self.semantic_indices]
        form_features = x[:, self.form_indices]
        
        semantic_out = self.semantic_network(semantic_features)
        form_out = self.form_network(form_features)
        
        semantic_form_concat = torch.cat([semantic_out, form_out], dim=1)
        
        combined_out = self.combined_projection(x)
        
        attention_weights = self.attention(semantic_form_concat)
        
        semantic_form_expanded = F.pad(semantic_form_concat, (0, 0), "constant", 0)
        
        semantic_weight = attention_weights[:, 0:1].expand_as(semantic_form_expanded)
        form_weight = attention_weights[:, 1:2].expand_as(semantic_form_expanded)
        combined_weight = attention_weights[:, 2:3].expand_as(combined_out)
        
        weighted_out = semantic_weight * semantic_form_expanded + form_weight * form_weight + combined_weight * combined_out
        
        output = self.final_network(weighted_out)
        
        return output, attention_weights

def build_enhanced_neural_network(input_shape, semantic_columns, all_columns, num_classes=2, hidden_layers=[256, 128, 64]):
    semantic_indices = [all_columns.index(col) for col in semantic_columns if col in all_columns]
    
    model = SimpleBalancedNetwork(input_shape, semantic_indices, hidden_layers, num_classes).to(DEVICE)
    
    print(f"Created enhanced balanced model with:")
    print(f"  - Semantic features: {len(semantic_indices)} ({semantic_columns})")
    print(f"  - Form features: {input_shape - len(semantic_indices)}")
...
def cross_validate_binary_classifier(directory_path="experiments/results/two_scores_with_long_text_analyze_2048T", 
                                    ...
                                    num_augmentations=2,
                                    noise_factor=0.05,
                                    use_balanced_architecture=True):
    ...
    semantic_features = []
    if 'score_chat' in features_df.columns:
        semantic_features.append('score_chat')
    if 'score_coder' in features_df.columns:
        semantic_features.append('score_coder')
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y_encoded)):
        ...
        if use_balanced_architecture and semantic_features:
            model = build_enhanced_neural_network(
...
def main():
    ...
    output_dir = 'models/balanced_binary_classifier'
    ...
    print(f"Best balanced binary model saved to {save_paths['model_path']}")

def visualize_attention_weights(model, X_test, y_test, label_encoder, output_dir='plots/binary'):
    model.eval()
    ...
    weights_by_class = {}
    for class_name in np.unique(y_test_labels):
        class_indices = np.where(y_test_labels == class_name)[0]
        class_weights = attention_np[class_indices]
        weights_by_class[class_name] = class_weights.mean(axis=0)
    
    plt.figure(figsize=(10, 6))
    bar_width = 0.35
    index = np.arange(3)
    channel_names = ['Semantic', 'Form', 'Full']
    
    for i, (class_name, weights) in enumerate(weights_by_class.items()):
        plt.bar(index + i*bar_width, weights, bar_width, label=class_name)
    
    plt.xlabel('Attention Channel')
    plt.ylabel('Average Attention Weight')
    plt.title('Attention Distribution by Class')
